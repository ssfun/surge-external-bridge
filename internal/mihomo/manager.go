...
func (m *Manager) ApplyProviders(definitions []ProviderDefinition) error {
	return m.ApplyProvidersBeforeCommit(definitions, nil)
}

// ApplyProvidersBeforeCommit prepares Providers without blocking publication.
// beforeCommit runs once, after preparation and before modifying live state.
// The caller may acquire its publication lock here and retain it until config
// persistence or rollback completes. It must not call back into Manager.
func (m *Manager) ApplyProvidersBeforeCommit(definitions []ProviderDefinition, beforeCommit func()) error {
	m.applyMu.Lock()
	defer m.applyMu.Unlock()
	m.mu.RLock()
...
func (m *Manager) ReorderProviders(definitions []ProviderDefinition) error {
	return m.ReorderProvidersBeforeCommit(definitions, nil)
}

// ReorderProvidersBeforeCommit uses the same publication guard contract as ApplyProvidersBeforeCommit.
func (m *Manager) ReorderProvidersBeforeCommit(definitions []ProviderDefinition, beforeCommit func()) error {
	m.applyMu.Lock()
	defer m.applyMu.Unlock()
	m.mu.RLock()
	cfg := m.config
...
func (m *Manager) ApplyProjectionSettings(bind, advertise string, port uint16, prefixProvider bool, masterKey []byte) error {
	return m.ApplyProjectionSettingsBeforeCommit(bind, advertise, port, prefixProvider, masterKey, nil)
}

// ApplyProjectionSettingsBeforeCommit acquires the publication guard only after
// waiting for other Provider work and preparing the candidate listener/snapshot.
func (m *Manager) ApplyProjectionSettingsBeforeCommit(bind, advertise string, port uint16, prefixProvider bool, masterKey []byte, beforeCommit func()) error {
