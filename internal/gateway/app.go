...
type App struct {
	mu               sync.RWMutex
	applyMu          sync.Mutex
	publicationMu    sync.RWMutex // guards config + projection commits; preparation stays outside
	store            *Store
	masterKey        []byte
	config           Config
	manager          *M.Manager
...
func (a *App) ReorderProviders(ids []string) error {
	a.applyMu.Lock()
	defer a.applyMu.Unlock()
	lockPublication, unlockPublication := a.publicationGuard()
	...
	if wasRunning {
		err = a.manager.ReorderProvidersBeforeCommit(definitions(candidate), lockPublication)
	} else {
		lockPublication()
...
func (a *App) UpdateSettingsPatch(settings Settings, managementToken, policyToken *string) error {
	...
	wasRunning := a.manager.Status().State == "running"
	if wasRunning {
		if err := a.manager.ApplyProjectionSettingsBeforeCommit(candidate.SocksBind, candidate.SocksHost, candidate.SocksPort, candidate.PrefixProvider, nextKey, lockPublication); err != nil {
			return err
		}
	} else {
		lockPublication()
...
func (a *App) applyConfig(candidate Config) error {
	...
	var applyErr error
	if wasRunning {
		applyErr = a.manager.ApplyProvidersBeforeCommit(definitions(candidate), lockPublication)
	} else {
		lockPublication()
...
// publicationGuard separates slow preparation from the committed config/runtime
// boundary. applyMu serializes writers; the guard is retained through persistence
// and rollback, including when Manager returns an error after acquiring it.
func (a *App) publicationGuard() (lock func(), unlock func()) {
	locked := false
	return func() { a.publicationMu.Lock(); locked = true }, func() {
		if locked {
